				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-right tong-tien"><strong></strong></td>
						</tr>
					</tbody>
				</table>
			</div>
		...
		<div class="col mb-2">
			<div class="row">
				<div class="col-sm-12 col-md-6">
					<button class="btn btn-block btn-light btn-delete-cartitem">Xoá</button>
				</div>
...
<script type="text/javascript">
	...
	function LoadCartItem() {
    	...
    	$.get('/sunnyFE/account/getsession', async function(ketqua) {
				...
				await fetch(url, {
					  ...
					  .then((data) => {
					    ...
					    if(status==200){
					    	...
					    	SoTien = formatmoney(SoTien, 'đ')
					    	GiamGia = formatmoney(GiamGia, '')
					    	DonGia = formatmoney(DonGia, 'đ')
								...
								<c:url value="/image?fname=\${item.productId.image.picture1}" var="imgUrl"></c:url>
								...
								<td id="product-delete" class="text-center">
									<button class="btn btn-sm btn-danger btn-delete-1-cartitem">
			...
			tongtien = formatmoney(tongtien, ' đ')
		   	$('.tong-tien strong').html(tongtien)
		});
	}
	...
	$(document).on('click','.btn-delete-cartitem', function() {
		...
		chooseRows.each(function () {
			...
			var found = listCartItem.filter(function(item) { return item.productId.pid === pid; });
			console.log(found)
			listchoose.push(found[0])
		})
		
		console.log(listchoose)
		
	    fetch('http://localhost:8083/cartitem/remove-list', {
		  method: 'DELETE',
		  headers: {
			    'Content-Type': 'application/json',
			  },
		  body: JSON.stringify(listchoose)
		})
		.then(r => r.json())
		.then(data => {
			LoadCartItem();
	...
	$(document).on('click','.btn-delete-1-cartitem', function() {
