...
public class loginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		else if (req.getParameter("shipper") != null) {
			JSONObject shipper = new JSONObject(req.getParameter("shipper")); 
			session.setAttribute("shipper", shipper);
			session.setAttribute("shipperName", shipper.get("name"));
			
		}
		else if (req.getParameter("Employee") != null) {
			JSONObject Employee = new JSONObject(req.getParameter("Employee")); 
			session.setAttribute("Employee", Employee);
			session.setAttribute("EmployeeName", Employee.get("name"));
